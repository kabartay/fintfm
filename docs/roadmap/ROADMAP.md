...
2. **Sweep `column_id_dim`** (39.30). §97 measured 12→16 as +0.0126 AP on 5 of 5 folds, and 16
   was chosen by accident. Two values have ever been tried; 20/24/32 are nearly free (§95).
3. **Learnability filter on the prior** (48.5). Nori filters tasks a simple learner cannot fit.
   §112 measured this prior's distinctiveness as weak, so the filter is aimed at a known gap.
4. **Cheap realism augmentations** (48.6): discretized features, noise, missingness. Published
   as worth a point or two, and no retrain of the architecture.
5. **Is-missing encoding** (48.10). Two peers disagree, so the answer is not knowable from
   reading and is cheap to measure.
6. **Schedule-free optimisation** (48.9). Decouples run length from a fixed schedule, which
   makes every later experiment cheaper even if it does not move rank itself.
7. **Random monotonic marginal augmentation** (40.8), motivated by a measured train/inference
   mismatch rather than by analogy.
8. **Second seed for the §91 ablation** (39.28). One run per arm is not a result, and the
...
9. **Write the factorized-attention cost model first** (44.1), and check it retrodicts linear
   CUDA and quadratic MPS growth (§95). §119 and §120 are both records of a cost model that was
   not checked before it was trusted.
10. **Establish what §91's mechanism requires** (44.2) before implementing anything.
11. **Implement the factorized encoder behind a flag** (44.3), defaulting off.
12. **Measure cost before accuracy** (44.4): peak training memory and step time.
13. **Then accuracy at the project's standard** (44.5): five-fold V4FinBench, paired bootstrap,
    Holm–Bonferroni.
14. **Re-test the levers it unblocks** (44.6): `max_context` past 2,000, model size past the
    current ceiling. These were closed under the current attention cost, not in principle.
15. **Joint objective, p(x,y|D)** (34.1): sum the objectives with a configurable weight.
16. **Pretrain at matched compute** (34.2) against `runs/v4-hazard-ldp.pt`.
17. **Score the joint checkpoint on the binary path** (34.3), Polish and Taiwan.
18. **Retire the two-checkpoint workflow if joint training wins** (34.4), and say so in the
...
19. **Labelled task-family generators** (40.2): linear, threshold, XOR, and the rest.
20. **Interaction-order curriculum, measured before touched** (40.3).
21. **Compositional generalisation test on existing checkpoints first** (40.4), matching 40.3's
    protocol so the two are comparable.
22. **Correlation, confounding and collider families** (40.5) on the existing SCM.
23. **Missingness, shift and support-extrapolation axes, sampled independently** (40.6).
24. **Widen target mechanisms to a published list** (48.4), so the comparison is against
    someone else's taxonomy rather than our own.
25. **Scope the combined prior only after 40.2–40.6 each validate individually** (40.7).
...
26. **Publish the multiclass checkpoint** measured in §121, and declare `multiclass`.
27. **Publish the regression checkpoint** measured in §121, and declare `regression`.
28. **Cost the two routes past the 136-feature cap against each other**: a checkpoint trained
...
29. **Re-run TabArena-Lite across all 51 datasets** and confirm the aggregate, remembering that
...
30. **Resubmit with all 51 datasets from the start**, as promised in the PR thread, and only
